...
class TcpTest(multinetwork_base.MultiNetworkBaseTest):
  ...
  def setUp(self):
    ...
    self.netid, self.iface1, self.iface2 = random.sample(self.tuns.keys(), 3)
    for netid in self.tuns:
      iface = self.GetInterfaceName(netid)
      self.SetSysctl("/proc/sys/net/ipv6/conf/%s/accept_ra" % iface, 2)
    self.SetSysctl("/proc/sys/net/ipv6/conf/all/forwarding", 1)
    self.iproute.IifRule(6, True,
                         self.GetInterfaceName(self.iface1),
                         self._TableForNetid(self.iface2), self.PRIORITY_IIF)

  def tearDown(self):
    for netid in self.tuns:
      iface = self.GetInterfaceName(netid)
      self.SetSysctl("/proc/sys/net/ipv6/conf/%s/accept_ra" % iface, 0)
    self.SetSysctl("/proc/sys/net/ipv6/conf/all/forwarding", 0)
    self.iproute.IifRule(6, False,
                         self.GetInterfaceName(self.iface1),
                         self._TableForNetid(self.iface2), self.PRIORITY_IIF)
