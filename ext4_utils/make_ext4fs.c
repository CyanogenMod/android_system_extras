...
/*
 * Transparent compression file format:
 *
 * byte[4]	magic		'zzzz'
 * byte		param1		method and flags
 * byte		blocksize	log2 of blocksize (min=8, max=31)
 * le48		original_size	original input size
 * if blocksize <= 16:
 *   le16	blockmap[blocks]
 * else:
 *   le32	blockmap[blocks]
 * byte		data[]
 *
 * blockmap is a vector containing the size of each compressed data block.
 * if a blockmap entry is zero, the block is not compressed.
 */
